    ...
    function periodic(){
        ...
        case _t.ST.E_NOACCOUNT:
            ...
            if(!err && accounts.length>0){
                _t.account=accounts[0];//アカウントの保存
                _t.st=_t.ST.READY;
                event(_t.st);
            }
            setTimeout(periodic,1000);
...
Web3Wrapper.prototype.account=undefined;
/**
...
{
    var _t=this;
    if(_t.st!=_t.ST.READY){
        throw new Error();
    }
    //from default,to toaccount
    var m = {to:toaccount, from:this.account,value: _t.web3.utils.toWei(amount, 'ether')};
    //送信
 ...
 * @return contract
 */
Web3Wrapper.prototype.newContract=function(_abi)
{
    console.log(this.web3);
    return new this.web3.eth.Contract(_abi);
}
